...
Gravity, Magnetics, Magnetic Resonance, VSP, straight-ray tomography, amplitude tomography
:::


::: {.callout-warning title="Can't we just do $\vb m = \vb G^{-1} \vb d$ ?"}
No, because  is usually not invertible, not even quadratic.
:::


## Types of inverse problems (naive)
...
$$\vb d = \vb G \vb m + \vb n \Rightarrow \vb G \in \mathfrak{R}^{N\times M}$$
:::
...
![Types of inverse problems [@Menke2012]](pics/bestimmtheit)
...
$$ \Phi = \| \vb d - \vb G \vb m \|^2_2 = \sum\limits_1^N (d_i-g_i({\vb m}))^2 \rightarrow\min $$
...
$$\mbox{with}\quad {\vb G}^\dagger = ({\vb G}^T{\vb G})^{-1} {\vb G}^T$$

This operator is called *generalized inverse* or *pseudo-inverse* (`np.linalg.pinv`).

The solution ${\vb m}_{LS}={\vb G}^\dagger {\vb d}$ is the **least-squares solution**, which can also be computed by `m=np.linalg.lstsq(G, d)` (in Matlab/Julia simply `G\d`).
Further derivations are at the bottom of the page.
